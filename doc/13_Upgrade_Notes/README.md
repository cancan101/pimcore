...
### [DataObject]
- [Relations] The many-to-one relation, many-to-many relation and advanced many-to-many relation data types have a new class definition option "Allow inline upload" (`assetInlineUploadAllowed`, `getAssetInlineUploadAllowed()` / `setAssetInlineUploadAllowed()`). It defaults to `true`, so existing class definitions keep showing the inline upload button of the relation field; uncheck it to only allow relating already existing assets.
