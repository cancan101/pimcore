...
### [Assets]
- [Thumbnails] `ImageThumbnailTrait::getStream()` (used by `Asset\Image\Thumbnail` and `Asset\Video\ImageThumbnail`) now returns `null` as documented instead of letting `League\Flysystem\UnableToReadFile` escape when the thumbnail file no longer exists on the thumbnail storage, e.g. because of a stale entry in the thumbnail status cache. The stale status cache entry is invalidated in that case, so the thumbnail is regenerated on the next request instead of failing again. `Asset\Service::getStreamedResponseFromImageThumbnail()` accordingly returns `null` for a missing stream and for a failed generation (previously a `TypeError` from writing a `null` stream to the storage) instead of throwing.
- [Thumbnails] **BC break:** The public thumbnail route now returns a `404 Not Found` when the requested asset or thumbnail configuration does not exist, or when the thumbnail can neither be delivered nor (re)generated — previously these cases redirected to the placeholder image. The placeholder redirect is now reserved for thumbnails whose generation actually failed (e.g. a broken source file) and for storage errors.
- [Thumbnails] **BC break:** A read failure for a thumbnail file that still exists on the thumbnail storage — or whose existence cannot be determined — is now surfaced as the underlying `League\Flysystem\UnableToReadFile` by `ImageThumbnailTrait::getStream()` and by the `@internal` `Asset\Service::getStreamedResponseForThumbnail()`, instead of being masked as a cache miss that silently triggered a regeneration on every request (the public thumbnail route responds with the placeholder redirect in that case). The public helper `Asset\Service::getStreamedResponseByUri()` keeps its nullable contract and returns `null` for such failures.
- [Thumbnails] **BC break:** When `ImageThumbnailTrait::getStream()` runs into a thumbnail file that is missing from the thumbnail storage, it now also resets the memoized state of the thumbnail instance and resolves the thumbnail again, i.e. it regenerates the missing file synchronously and serves it right away (previously the read failure was thrown). Subsequent calls like `getPath()`, `exists()` or `getFileSize()` on the same instance therefore reflect the regenerated thumbnail instead of the stale path reference. Custom classes using the trait that rely on `getStream()` never triggering a generation need to be adapted.
